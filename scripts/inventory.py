...
def main():
    ...
    limits = ''.join('<li>'+html.escape(Path(x['root']).name)+': excluded '+str(x['excluded_count'])+
        ', errors '+str(len(x['errors']))+', '+('scan complete' if x['complete'] else 'PARTIAL')+'</li>' for x in results)
    page = '<!doctype html><html lang="en"><meta charset="utf-8"><title>Local inventory</title><h1>Local inventory</h1><p>Metadata only. No source-content reads, network calls or hashes. Symlinks are not followed. Hidden and restricted names are excluded. Counts are not deduplicated; this is not deletion approval.</p><table><tr><th>Source</th><th>Directory</th><th>Files</th><th>Images</th><th>Links</th><th>GB</th></tr>' + ''.join(rows) + '</table><h2>Scan coverage</h2><ul>' + limits + '</ul><p><a href="inventory.json">Local metadata</a>. Keep this report out of public Git.</p></html>'

...
def main():
    ...
    labels = {'missing': 'Missing', 'present_unverified': 'Present locally; content unverified',
              'empty_file': 'Empty files', 'not_regular_file': 'Links or special files',
              'ambiguous_local_files': 'Multiple files for one ID', 'duplicate_manifest_id': 'Duplicate manifest ID'}
    ...
    page = '<!doctype html><html lang="en"><meta charset="utf-8"><title>Image reconciliation</title><h1>Image reconciliation</h1><p>Offline filename matching only. Nonempty regular files are not verified copies. No image contents read, downloads, uploads or deletions.</p><table>' + summary + '</table><p>Manifest rows: ' + str(result['manifest_rows']) + '; unique IDs: ' + str(result['unique_ids']) + '; rows without source URL: ' + str(result['rows_without_source_url']) + '; local IDs outside manifest: ' + str(len(result['local_ids_outside_manifest'])) + '.</p><h2>Sources</h2><p>' + html.escape(str(manifest)) + '; ' + html.escape(str(images)) + '</p><p><a href="reconcile.json">Full local result</a>. Keep out of public Git.</p><h2>Missing or requiring review</h2><table>' + detail + '</table></html>'
